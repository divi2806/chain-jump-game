class SpaceJumpGameClass {
    ...
    loadAssets() {
        ...
        imageAssets.forEach(filename => {
            ...
            img.onload = () => {
                this.assetsLoaded++;
                console.log(`Loaded image: ${filename} (${this.assetsLoaded}/${this.totalAssets})`);
            ...
            img.onerror = (e) => {
                console.warn(`Failed to load image asset: ${filename}`, e);
                img.error = true; // Mark as errored
                this.assetsLoaded++;
                if (this.assetsLoaded === this.totalAssets) {
        ...
        soundAssets.forEach(filename => {
            ...
            audio.oncanplaythrough = () => {
                this.assetsLoaded++;
                console.log(`Loaded audio: ${filename} (${this.assetsLoaded}/${this.totalAssets})`);
                if (this.assetsLoaded === this.totalAssets) {
                    console.log('Assets loaded, starting game');
                    this.init();
                }
            ...
            audio.onerror = (e) => {
                console.warn(`Failed to load sound asset: ${filename}`, e);
                audio.error = true; // Mark as errored
                this.assetsLoaded++;
                if (this.assetsLoaded === this.totalAssets) {
                    console.log('Game starting (some assets failed)');
                    this.init();
        ...
        if (currentBg && 
            ...
            try {
                ...
                if (nextBg && 
                    ...
                    nextBg.naturalHeight > 0 &&
                    bgInfo.transitionProgress > 0.2) {
                    ...
                    try {
                        ...
                        console.warn('Failed to draw next background, skipping transition:', e);
                    }
                }
                ...
                console.warn('Failed to draw current background, using fallback:', e);
    ...
    renderPlatforms() {
        this.platforms.forEach(platform => {
            ...
            if (screenY > -50 && screenY < this.canvas.height + 50) {
                ...
                if (this.assets[platform.asset] && 
                    this.assets[platform.asset].complete && 
                    !this.assets[platform.asset].error &&
                    this.assets[platform.asset].naturalWidth > 0 &&
                    this.assets[platform.asset].naturalHeight > 0) {
                    ...
                    try {
                        ...
                        console.warn(`Failed to draw platform asset ${platform.asset}, using fallback:`, e);
                        // Use fallback rendering below
                        this.renderPlatformFallback(platform, screenY);
                    }
                } else {
                    this.renderPlatformFallback(platform, screenY);
                }
    ...
    renderPlatformFallback(platform, screenY) {
        ...
        this.coins.forEach(coin => {
            if (!coin.collected) {
                ...
                if (screenY > -50 && screenY < this.canvas.height + 50) {
                    ...
                    if (this.assets.coin && 
                        this.assets.coin.complete && 
                        !this.assets.coin.error &&
                        this.assets.coin.naturalWidth > 0 &&
                        this.assets.coin.naturalHeight > 0) {
                        ...
                        try {
                            ...
                            console.warn('Failed to draw coin sprite, using fallback:', e);
        ...
        this.shoes.forEach(shoes => {
            if (!shoes.collected) {
                ...
                if (screenY > -50 && screenY < this.canvas.height + 50) {
                    ...
                    // Validate image before drawing
                    if (this.assets.shoes && 
                        this.assets.shoes.complete && 
                        !this.assets.shoes.error &&
                        this.assets.shoes.naturalWidth > 0 &&
                        this.assets.shoes.naturalHeight > 0) {
                        ...
                        try {
                            ...
                            console.warn('Failed to draw shoes sprite, using fallback:', e);
                            // Use fallback rendering
    ...
    renderPlayer() {
        ...
        // Validate image before drawing
        if (this.assets.astronaut && 
            this.assets.astronaut.complete && 
            !this.assets.astronaut.error &&
            this.assets.astronaut.naturalWidth > 0 &&
            this.assets.astronaut.naturalHeight > 0) {
            ...
            try {
                ...
                console.warn('Failed to draw astronaut sprite, using fallback:', e);
                // Use fallback rendering
